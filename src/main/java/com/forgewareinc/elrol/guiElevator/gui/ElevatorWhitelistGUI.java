...
public class ElevatorWhitelistGUI extends GuiScreen{
	...
	public ElevatorWhitelistGUI (World world, int x, int y, int z, EntityPlayer player){
		...
		this.player = player;
		this.world = world;
		te = (TileEntityElevator)world.getTileEntity(x, y, z);
	...
	public void initGui(){
		...
		buttonList.clear();
		nameInput = new GuiTextField(this.fontRendererObj, guiX+12, guiY+20, 100, 20);
	...
	protected void actionPerformed(GuiButton button){
		...
		case 0:
			if(!nameInput.getText().trim().equals("")){
				...
				((TileEntityElevator)world.getTileEntity(X, Y, Z)).addUsername(this.nameInput.getText().trim());
				((TileEntityElevator)world.getTileEntity(X, Y, Z)).markDirty();
		...
		case 1:
			...
			break;
		case 2:
			((TileEntityElevator)world.getTileEntity(X, Y, Z)).removeUsername(selected-5);
			((TileEntityElevator)world.getTileEntity(X, Y, Z)).markDirty();
